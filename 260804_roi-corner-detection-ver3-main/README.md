...
원본 repository는 [nampluskr/roi-corner-detection-ver3](https://github.com/nampluskr/roi-corner-detection-ver3)다.
...
| --- | --- | --- | --- |
| `reg` | normalized corner 8개 직접 회귀 | `spatial`, `gap` | [Regression](docs/models/01-reg.md) |
...
Python 실행과 검증은 `windows-office`의 WinPython interpreter와 project root를 기준으로 한다.

```powershell
& "C:\winpython\WPy64-310111dot\python-3.10.11.amd64\python.exe" scripts/train.py --help
```
...
python scripts/evaluate.py \
  ...
  --num_workers 0 \
  --checkpoint outputs/public/reg/quickstart/model.pth \
  --output_dir outputs/public/reg/quickstart
```
...
| `--head` | `spatial` | model-specific variant |
...
global `network=custom`, `head=spatial` default는 `reg`에 맞는다. 다른 model을 선택할 때는 compatible head와
network를 명시한다. 전체 option과 예시는 [CLI Usage Guide](docs/guides/02-cli-usage.md)를 참고한다.

stage별 batch configuration은 `configs/*.py`의 `BASE["image_size"]`를 사용한다. 이 값은 batch runner를 통해
data loader resize와 wrapper의 image-size 의존 target 및 postprocess에 함께 전달된다. 현재 stage config의
기본값은 512이지만, 직접 CLI 실행에서 option을 생략하면 parser 기본값 224를 사용한다.

`scripts/train.py`는 `--dataset` 값에 따라 이전 단계 output 경로의 `model.pth`가 있으면 그 weight를
자동으로 불러와 학습을 시작한다. 단계 선후 관계는 `synthetic`의 이전 단계가 `public`, `measured`의 이전
단계가 `synthetic`이며, `public`이거나 이전 단계 `model.pth`가 없으면 backbone init에서 학습한다.
`evaluate.py`와 `predict.py`에서 `--checkpoint`를 생략하면 현재 stage output 경로의 `model.pth`를
기본 checkpoint로 사용한다.
...
`exp_name`은 `<model>_<network>_<head>_<dataset>`다.

automatic name에는 CSV, seed, image size, subset size, batch size, epoch, patience, warmup이 포함되지 않는다. 이 값만 다른
...
- `--image_size`는 data loader resize와 wrapper의 image-size 의존 target 및 postprocess에 함께 적용된다.
  학습, 평가, 예측에는 같은 값을 사용해야 한다.
- checkpoint는 model state dictionary만 저장하며 optimizer, scheduler, epoch와 config를 포함하지 않는다.
- training script의 `--checkpoint`는 resume load가 아니라 `--save`와 함께 사용할 save path다. 이전 단계
  weight 초기화는 `--dataset`에 따라 자동으로 이루어진다.
- 이전 단계 초기화는 optimizer와 scheduler 상태를 이어받지 않고 model weight만 불러오며, 단계별 learning
  rate 스케줄을 자동 적용하지 않는다.
